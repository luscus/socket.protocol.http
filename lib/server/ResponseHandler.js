...
exports.onMessage = function onMessage (packet, response, socket, raw, meta) {
  if (meta.path === '/') {
    socket.lib._respond({error: 'provide an url path'}, response);
    return;
  }
  ...
  if (packet) {
    ...
    socket.lib._respond(packet, meta, response);
  }
  else {
    socket.lib._respond({reason: 'no data was provided'}, meta, response, 400);
  }
};
...
exports.onError   = function onError (request, response, route, error) {
  ...
  if (ErrorHandler[error.code]) {
    ...
    socket.lib._respond({error: error.message, stack: error.stack}, response);
  }
};
...
exports.handler   = function handler (request, response) {
  ...
  if (packet.CONNTEST) {
    socket.lib._respond(packet, null, response);
    return;
  }
